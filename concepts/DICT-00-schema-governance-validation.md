...
v1.2 additive — EODHD full-surface expansion: 15 capability audit & dispositions · 16 ownership/insider/analyst/ESG · 17 news & sentiment · 18 options-implied · 19 credit/rates/sovereign/country · WORKED-EXAMPLES-3 (4 artifacts). New governance rules: single-source quality cap (DICT-15 §4), methodology-as-identity for opinion scores (no cross-vendor quorum for esg.*/sov.default_spread-class concepts), and the standing rule that an honest empty vendor_map beats a manufactured KT.
v1.4 additive — OSAP catalog audit: 22 alpha-catalog mapping (212 Chen–Zimmermann predictors dispositioned; per-predictor map is data in mappings/). New surfaces: C2 recipe primitives (§3 of 22), OHLC/expense/backlog raw items, indicator.mohanram_g, opt.volume_ratio, credit.issuer_rating, est.ltg_mean, and the factor.* internal factor-return family (frozen construction over a named universe_id, model_id in provenance — never vendor factor data). Catalog imports carry {osap_acronym, authors, year} provenance; upstream OSAP numbers are replication benchmarks only, never ingested values.
v1.3 additive — ad-hoc/long-tail track: 20 framework (subject kinds beyond entities, link.exposure linkage concepts, dataset manifests, sandbox→probation→graduated lifecycle) · 21 cmdty/trade/product domains. New schema surface: subject_kind generalizes the index key; `adhoc.` prefix is a governed quarantine namespace; linkage (link.*) is data with versions and evidence, never inline joins. Separate entry gate, same serving contract — ad-hoc data flows through the identical DAL/vintage/provenance machinery.
